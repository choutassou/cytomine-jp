...
public class RelationTermService extends ModelService {
    ...
    public List<RelationTerm> list(Term term, String position) {
        ...
        return position.equals("1")
            ? relationTermRepository.findAllByTerm1(term)
            : relationTermRepository.findAllByTerm2(term);
    }
    ...
    public void checkDoNotAlreadyExist(CytomineDomain domain) {
        ...
        if (relationTerm != null
            && relationTerm.getRelation() != null
            && relationTerm.getTerm2() != null
            && relationTerm.getTerm2() != null) {
            if (relationTermRepository.findByRelationAndTerm1AndTerm2(
                relationTerm.getRelation(),
                relationTerm.getTerm1(),
                relationTerm.getTerm2()
            ).stream().anyMatch(x -> !Objects.equals(x.getId(), relationTerm.getId()))) {
                throw new AlreadyExistException("RelationTerm with relation="
                    + relationTerm.getRelation().getId()
                    + " and term1 "
    ...
    public CytomineDomain retrieve(JsonObject json) {
        return relationTermRepository.findByRelationAndTerm1AndTerm2(
                json.getJSONAttrLong("relation"),
                json.getJSONAttrLong("term1"),
                json.getJSONAttrLong("term2")
            )
